 ...
 * =================================================================================================
 *
 * deploy-ir-schema.js (for Browser DevTools)
 *
 * This Javascript snippet is designed to be run directly in the browser's developer console.
 * It deploys the Innovation Record (IR) schema by taking a globally defined schema object
 * and POSTing it to the backend's admin endpoint.
 *
 * **PREREQUISITES:**
 * 1. You must be logged into the application frontend in the current browser tab with an
 *    administrator account. The script uses the browser's active session cookie for authentication.
 * 2. The `IR_SCHEMA` object (as a JSON object) must be available as a global variable
 *    named `schemaToDeploy` in the console scope.
 *
 * **WHAT IT DOES:**
 * 1. It takes the globally available `schemaToDeploy` object.
 * 2. It makes a `fetch` request to the `/api/v1/ir-schema` endpoint.
 * 3. The browser automatically attaches the necessary `connect.sid` cookie for authentication.
 * 4. It logs the result of the operation to the console.
 *
 * **ARGUMENTS (to be configured in the script):**
 *   - `TARGET_URL`: The base URL of the target backend. Defaults to the current origin,
 *     but can be overridden for different environments (e.g., 'https://your-prod-api.com').
 *
 * =================================================================================================
 */
(async function deploySchema() {
  const schemaToDeploy = {
    sections: [
      {
  ...
  try {
    ...
    if (response.ok && response.status === 204) {
      console.log("%cSUCCESS: Schema deployed successfully.", "color: green; font-weight: bold;");
    } else {
