...
import { randBoolean, randEmail, randFullName, randUuid } from '@ngneat/falso';
...
const expected: Awaited<ReturnType<UsersService['getUserInfo']>> = {
  id: randUuid(),
  email: randEmail(),
...
describe('v1-admin-user-info Suite', () => {
  const johnInnovator = scenario.users.johnInnovator;

  describe('200', () => {
    it.each([
      ['id', johnInnovator.id],
      ['email', johnInnovator.email]
    ])('should return user info when %s is passed', async (_type: string, idOrEmail: string) => {
      const result = await new AzureHttpTriggerBuilder()
        .setAuth(scenario.users.allMighty)
        .setParams({ userIdOrEmail: idOrEmail })
  ...
  describe('Access', () => {
    it.each([
      ...
      const result = await new AzureHttpTriggerBuilder()
        .setAuth(user)
        .setParams({ userIdOrEmail: johnInnovator.id })
