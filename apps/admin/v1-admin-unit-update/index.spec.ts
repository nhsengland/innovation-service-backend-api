...
const mock = jest.spyOn(OrganisationsService.prototype, 'updateUnit').mockResolvedValue(expected);

afterEach(() => {
  jest.clearAllMocks();
});

describe('v1-admin-unit-update Suite', () => {
  describe('200', () => {
    it('should update the unit', async () => {
      ...
      expect(result.body).toStrictEqual({ unitId: expected.id });
