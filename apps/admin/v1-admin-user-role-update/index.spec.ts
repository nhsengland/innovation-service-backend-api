...
const mock = jest.spyOn(UsersService.prototype, 'updateUserRole').mockResolvedValue();

afterEach(() => {
  jest.clearAllMocks();
});

describe('v1-admin-user-role-update Suite', () => {
  describe('204', () => {
    it('should update the role', async () => {
      const result = await new AzureHttpTriggerBuilder()
        .setAuth(scenario.users.allMighty)
        .setParams({ userId: randUuid(), roleId: randUuid() })
        .setBody<BodyType>({
          enabled: randBoolean()
        })
