...
const createActionSpy = jest.spyOn(InnovationActionsService.prototype, 'createAction').mockResolvedValue(expected);
  ...
  describe('400', () => {
    it.each(['section', 'description'] as const)('should fail if missing mandatory parameter %s', async parameter => {
      ...
      const result = await new AzureHttpTriggerBuilder()
        ...
        .call<ErrorResponseType>(v1InnovationActionCreate);
