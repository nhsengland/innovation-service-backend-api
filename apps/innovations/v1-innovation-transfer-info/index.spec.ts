...
const expected = {
  id: randUuid(),
  email: randEmail(),
  innovation: { id: randUuid(), name: randText(), owner: { name: randFullName() } }
};

const mock = jest.spyOn(InnovationTransferService.prototype, 'getInnovationTransferInfo').mockResolvedValue(expected);

afterEach(() => {
  jest.clearAllMocks();
});

describe('v1-innovation-transfer-info Suite', () => {
  describe('200', () => {
    it('should return the transfer info', async () => {
