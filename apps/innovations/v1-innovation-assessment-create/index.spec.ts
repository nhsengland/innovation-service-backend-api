import v1InnovationAssessmentCreate from '.';
...
const mock = jest
  .spyOn(InnovationAssessmentsService.prototype, 'createInnovationAssessment')
...
describe('v1-innovation-assessment-assessor-update Suite', () => {
  const innovationId = scenario.users.ottoOctaviusInnovator.innovations.powerSourceInnovation.id;
  describe('200', () => {
    it('should return success', async () => {
      const result = await new AzureHttpTriggerBuilder()
        ...
        .call<ResponseDTO>(v1InnovationAssessmentCreate);
  ...
  describe('Access', () => {
    ...
    it('fails if innovation status != "WAITING_NEEDS_ASSESSMENT"', async () => {
      const result = await new AzureHttpTriggerBuilder()
        .setAuth(scenario.users.paulNeedsAssessor)
