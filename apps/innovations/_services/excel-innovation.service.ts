...
export class ExcelInnovationService {
   ...
   */
  async importInnovation(
    domainContext: DomainContextType,
    base64Xlsx: string
  ): Promise<{ id: string; validationIssues: Record<string, string[]>; emptySections: string[] }> {
    ...
    if (!regPayload["name"] || !regPayload["description"] || !regPayload["officeLocation"]) {
      console.error(
        "[ExcelImport] Missing required fields. Name:",
        regPayload["name"],
        "Description:",
        regPayload["description"],
        "Location:",
        regPayload["officeLocation"]
      );
    ...
    const { id: innovationId } = await this.innovationsService.createInnovation(domainContext, regPayload as any);
   ...
   */
  async importInnovationFromJson(
    domainContext: DomainContextType,
    jsonPayload: Record<string, Record<string, any>>
  ): Promise<{ id: string; validationIssues: Record<string, string[]>; emptySections: string[] }> {
    const schema = await this.irSchemaService.getSchema();
    ...
    if (!regPayload["name"] || !regPayload["description"] || !regPayload["officeLocation"]) {
      console.error(
        "[JsonImport] Missing required fields. Name:",
        regPayload["name"],
        "Description:",
        regPayload["description"],
        "Location:",
    ...
    for (const section of schema.model.schema.sections) {
      for (const subSection of section.subSections) {
        ...
        sections.push({
          ...
          calculatedFields,
          finalPayload,
          validationIssues: secValidationIssues
        });
      }
    }
  ...
  private async saveImportedSections(
    domainContext: DomainContextType,
    innovationId: string,
    sections: any[]
  ): Promise<void> {
