...
export class StatisticsHandlersHelper {
  ...
  static async runHandler(
    ...
    const handlers = actions.map(async (action) => {
      const handler = await USER_STATISTICS_CONFIG[action].handler(requestUser);
  ...
  private static buildResponse(
    statistics: { action: UserStatisticsEnum, data: (UserStatisticsTemplateType[UserStatisticsEnum])}[]): { [key: string]: UserStatisticsTemplateType[UserStatisticsEnum]; } {
