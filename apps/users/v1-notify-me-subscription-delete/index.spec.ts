...
const mock = jest.spyOn(NotifyMeService.prototype, 'deleteSubscription').mockResolvedValue();

afterEach(() => {
  jest.clearAllMocks();
});

describe('v1-notify-me-subscription-delete Suite', () => {
  describe('204', () => {
    it('should delete notify me subscription', async () => {
      const result = await new AzureHttpTriggerBuilder()
        .setAuth(scenario.users.aliceQualifyingAccessor)
        .setParams<ParamsType>({ subscriptionId: randUuid() })
