...
describe('identity-name helper', () => {
  it('merges a given-name-only update and derives displayName', () => {
    expect(normalizeNameUpdate({ givenName: 'Jonathan' }, { givenName: 'John', surname: 'Smith' })).toMatchObject({
      givenName: 'Jonathan',
      surname: 'Smith',
  ...
  it('merges a surname-only update and derives displayName', () => {
    expect(normalizeNameUpdate({ surname: 'Smythe' }, { givenName: 'Jonathan', surname: 'Smith' })).toMatchObject({
      givenName: 'Jonathan',
      surname: 'Smythe',
  ...
  it('rejects a name update when the merged name pair is incomplete', () => {
    expect(() => normalizeNameUpdate({ givenName: 'Jonathan' }, { givenName: '', surname: '' })).toThrow(
