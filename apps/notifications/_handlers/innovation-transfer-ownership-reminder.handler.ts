...
> {
  ...
  async run(): Promise<this> {
    const targetUser = await this.identityProviderService.getUserInfoByEmail(
      this.inputData.recipientEmail
    );
    ...
    if (!targetUser) {
      ...
      if (userId) {
        const userRole = await this.recipientsService.getUserRole(
          userId,
