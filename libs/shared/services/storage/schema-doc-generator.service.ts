...
export class SchemaDocGeneratorService {
  ...
  private extractQuestionExample(q: Question, payload: Record<string, any>): void {
    ...
    if (dt === 'text' || dt === 'textarea') {
      ...
      const options = resolveQuestionItems(q as RadioGroup)
        .map(i => i.id || i.label)
        .filter(Boolean);
      ...
      if (fg.addQuestions?.length) {
        const addQuestionFields = fg.addQuestions.map(addQuestion => `"${addQuestion.id}": "string"`).join(', ');
        valRepresentation = `[ { "${fg.field.id}": "string", ${addQuestionFields} } ]`;
      }
    }
  ...
  private renderQuestionTableRow(q: Question, depth = 0, conditionMsg = ''): string {
    ...
    // Handle sub-questions (checkbox array addQuestions)
    if (dt === 'checkbox-array') {
      const cond = `*(Sub-answer for selected items)*`;
      for (const addQ of (q as CheckboxArray).addQuestions ?? []) {
    ...
    if (dt === 'fields-group') {
      ...
      for (const addQ of fg.addQuestions ?? []) {
        row += this.renderQuestionTableRow(addQ, depth + 1, '*(Sub Field)*');
      }
    }
