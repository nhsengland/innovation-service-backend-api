...
export class ExcelExportService {
  ...
  private renderQuestionDispatcher(
    ...
    switch (question.dataType) {
      ...
      case 'autocomplete-array':
        ...
        break;
      case 'checkbox-array':
        mainCellAddr = this.renderCheckboxArrayQuestion(
          sheet,
          refSheet,
          question as CheckboxArray,
          isIndented,
          ssPayload
        );
        break;
    ...
    resolvedItems.forEach((item: any) => {
      ...
      if (item.label || item.id) {
        ...
        if (question.addQuestion && question.addQuestion.label) {
          subLabel =
            question.addQuestion.label.replace(/\{\{item.*?\}\}/g, item.label || item.id) +
   ...
   */
  private renderFieldsGroupQuestion(
    sheet: ExcelJS.Worksheet,
    refSheet: ExcelJS.Worksheet,
    ...
    sheet.addRow(['']);
    // Header
    sheet.addRow(['', this.getLabel(question, isIndented), '', '', this.getGuidance(question), question.id]).font = {
      italic: true,
      bold: true
    };
