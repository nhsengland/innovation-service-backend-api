...
export class ExcelExportService {
   ...
   */
  private renderSectionHeader(sheet: ExcelJS.Worksheet, title: string): void {
   ...
   */
  private renderSubSectionHeader(sheet: ExcelJS.Worksheet, title: string): void {
    const row = sheet.addRow([title, '', '', '', '', '', '']);
    sheet.mergeCells(`A${row.number}:E${row.number}`);
    row.height = 32;
   ...
   */
  private applyInputStyle(cell: ExcelJS.Cell): void {
   ...
   */
  private applyCascadingRed(sheet: ExcelJS.Worksheet, currentAddr: string, isRequired: boolean): void {
