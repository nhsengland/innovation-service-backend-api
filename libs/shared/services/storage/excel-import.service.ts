...
import type { Question, CheckboxArray } from '../../models/schema-engine/question.types';
...
const COL_SUB = 4; // D — legacy sub-answer column, unused for static Excel import
...
export class ExcelImportService {
   ...
   *   3. Extracts answers by supported dataType (text, radio, checkbox)
   ...
   */
  public parseWorkbook(workbook: ExcelJS.Workbook, schema: any, schemaModel: SchemaModel): ExcelImportResult {
   ...
   * - `radio-group`: Single choice dropdown with conditional child recursion.
   * - `autocomplete-array`: Multi-select or single-choice depending on validations.
   * - `checkbox-array`: Multi-select; questions with `addQuestions` are skipped.
   *
   * @example
  ...
  private extractQuestions(
    ...
    for (const question of questions) {
      if (isQuestionUnsupportedForExcel(question)) continue;
      ...
      if (dt === 'text' || dt === 'textarea') {
        ...
        const { answers } = this.readCheckboxQuestion(sheet, rowIndex, question as CheckboxArray);
        if (answers.length > 0) {
  ...
  private readTextQuestion(
    ...
    if (answerVal) {
      console.log(
        `[ExcelImport] Text Fallback Success: Found "${answerVal}" in Column C for ${question.id} at row ${rowNum}`
      );
      return answerVal;
    }
    ...
    if (subAnswerVal) {
      console.log(
        `[ExcelImport] Text Fallback Success: Found "${subAnswerVal}" in Column D for ${question.id} at row ${rowNum}`
      );
      return subAnswerVal;
    }
  ...
  private readRadioQuestion(
    ...
    if (label) {
      ...
      if (found) {
        console.log(
          `[ExcelImport] Radio Fallback Success: Matched label "${label}" to ID "${found.id || found.label}" for question ${question.id}`
        );
   ...
   * 4. Dynamic child answers are not extracted; their parent is unsupported.
   *
   * @example
   * // User marks "Medical Device" and "AI" as "Selected".
   * // Returns: { answers: ["MEDICAL_DEVICE", "AI"] }
   *
   * @param sheet - The Excel worksheet.
   * @param rowIndex - The row lookup index.
