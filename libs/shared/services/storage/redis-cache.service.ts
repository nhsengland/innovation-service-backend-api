...
export class RedisCache<T> {
  ...
  async getMany(keys: string[]): Promise<(T)[]> {
    try {
      return (await this.redis.mGet(keys.map(key => `${this.name}_${key}`))).filter(((item): item is string => item !== null)).map(item => JSON.parse(item) as T);
    } catch (e) {
  ...
  async setMany(values: {key: string, value: T}[], _ttl?: number): Promise<void> {
    try {
      ...
      for(const value of values) {
        ...
        await this.redis.set(`${this.name}_${value.key}`, JSON.stringify(value.value), {EX: _ttl ?? this.ttl});
      }
    } catch (e) {
      this.logger.log(`Error setting keys ${values.map(({key}) => key)} in cache ${this.name}: ${e instanceof Error && e.message}`);
    }
  }
