...
import { InnovationEntity, UserRoleEntity, NotificationEntity } from '../../entities';
...
export class NotificationBuilder extends BaseBuilder {
  ...
  async save(): Promise<TestNotificationType> {
    ...
    const result = await this.getEntityManager().createQueryBuilder(NotificationEntity, 'notification')
    ...
    const resultNotificationUsers = await this.getEntityManager().createQueryBuilder(NotificationUserEntity, 'notificationUser')
        .innerJoinAndSelect('notificationUser.userRole', 'role')
        .where('notificationUser.id IN (:...notificationUserIds)', { notificationUserIds: savedNotificationUsers.map(nU => nU.id)})
        .getMany();
