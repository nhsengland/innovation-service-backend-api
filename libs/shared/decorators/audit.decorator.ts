...
import { AuditServiceSymbol, AuditServiceType, SQLConnectionServiceSymbol, SQLConnectionServiceType } from '../services/interfaces';
...
type AuditOptions = {action: ActionEnum, target: TargetEnum} & ({identifierParam: string} | {identifierQuery: string} | {identifierBody: string} | {identifierResponseField: string});
    ...
    descriptor.value = async function (context: CustomContextType, request: HttpRequest) {
      ...
      for (const option of auditOptions) {
        ...
        if('identifierParam' in option) {
          ...
          throw new NotImplementedError(GenericErrorsEnum.NOT_IMPLEMENTED_ERROR, {message: 'Audit decorator only supports identifierParam and identifierResposneField at the moment.'});
        }
        ...
        const innovationId = option.target === TargetEnum.INNOVATION ? targetId : request.params['innovationId'] || request.query['innovationId'] || request.body['innovationId'];

        // Get the user id from the externalId
        const user = (await sqlConnection.createQueryBuilder().select('id').from('user', 'user').where('user.external_id = :externalId', {externalId: context.auth.user.identityId}).getRawOne())?.id;
