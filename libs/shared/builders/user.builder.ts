...
export class UserBuilder extends BaseBuilder {
  ...
  addRole(type: ServiceRoleEnum, organisation?: OrganisationEntity, organisationUnit?: OrganisationUnitEntity): this {
    ...
    if((type === ServiceRoleEnum.ACCESSOR || type === ServiceRoleEnum.QUALIFYING_ACCESSOR) && !organisation && !organisationUnit) throw new Error('Accessor role must have both organisation and organisation unit');
    this.user.serviceRoles.push(UserRoleEntity.new({ role: type, organisation: organisation ?? null, organisationUnit: organisationUnit ?? null }));
    return this;
  }
