 ...
 */
export const resolveNestedPromises = async <T extends Record<string, any>>(entity: T, typeOrm = true): Promise<ResolvedNestedPromise<T>> => {
  ...
  for(const key of Object.keys(entity)) {
    ...
    if (value instanceof Array) {
      (plain as any)[myKey] = await Promise.all(value.map(async (item) => {
