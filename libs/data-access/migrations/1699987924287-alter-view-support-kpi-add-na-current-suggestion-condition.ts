...
// activity log
export class alterViewSupportKPIAddNACurrentSuggestedCondition1699987924287 implements MigrationInterface {
  async up(queryRunner: QueryRunner): Promise<void> {
    ...
    WITH latest_suggested AS (
      ...
      LEFT JOIN innovation_assessment a ON a.innovation_id = i.id AND a.deleted_at IS NULL
	    LEFT JOIN innovation_assessment_organisation_unit aou ON aou.innovation_assessment_id = a.id AND aou.organisation_unit_id = slou.organisation_unit_id
      WHERE s.id IS NULL
      -- only consider currently suggested units for ASSESSMENT_SUGGESTION (2 different behaviours for ASSESSMENT_SUGGESTION and ACCESSOR_SUGGESTION)
      AND (
        (sl.type = 'ASSESSMENT_SUGGESTION' AND aou.organisation_unit_id IS NOT NULL)
        OR (sl.type = 'ACCESSOR_SUGGESTION')
      )
